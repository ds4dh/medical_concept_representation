...
def compute_prediction_binary(
    ...
    for n, (sample, gold) in loop:
        ...
        patient_embeddings.append(
            get_patient_embedding(model, sample, pipeline, random_mode)
        )
        golds.append(gold)
...
def get_patient_embedding(
    ...
    fixed_enc = [
        encoded[n] for n, t in enumerate(sample)
...
def generate_csv(
    ...
    for (cat, multi_results), (_, binary_result) in\
        ...
        new_rows = [[cat, str(p)] for p in PARTIAL_LEVELS]
    ...
    csv_filepath = "prediction_results_rd_%s_rc_%s.csv" % (REDUCED_DIM, random_mode)
